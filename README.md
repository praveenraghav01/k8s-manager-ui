...
Releases are built and published by the [`Build & Release`](.github/workflows/release.yml) GitHub Actions workflow — a parallel matrix across macOS, Windows and Linux that runs the test suite, packages each installer, and attaches them to a `v<VERSION>` GitHub Release with **auto-generated release notes** (from the PRs and commits merged since the previous tag). The `VERSION` file is the single source of truth for the version.

Trigger it either way:

- **Tag push** — `git tag vX.Y.Z && git push origin vX.Y.Z`.
- **Manual** — Actions → **Build & Release** → **Run workflow** on `main` (reads `VERSION`; an optional input overrides the tag).

> [!NOTE]
> To re-publish the **same** version (e.g. ship a fix without a version bump), run the workflow on `main` without editing `VERSION` — it rebuilds and replaces the installers on the existing `v<VERSION>` release. Notes are generated by GitHub, so there is no changelog file to maintain.
